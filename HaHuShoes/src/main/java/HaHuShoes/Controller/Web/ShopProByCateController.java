...
@WebServlet(urlPatterns = {"/shopprobycate"})
public class ShopProByCateController  extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		int	count = productService.countProductByCategory(Integer.parseInt(cateId));
		products = productService.findProductbyCateId(Integer.parseInt(cateId), index);
		int endP = count/3;
		if(count % 3 != 0)
		{
			endP++;
		...
		req.getRequestDispatcher("/views/web/shopprobycate.jsp").forward(req, resp);
	}
