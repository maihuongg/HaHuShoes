				...
				<tbody class="align-middle">
					<c:forEach items="${orderList}" var="i">
						<tr>
							...
							<c:if test="${i.prostatus ==1 }">
										<td>Đang giao<hr/>
										<a class="btn btn-sm btn-primary"
										href="<c:url value='/confirm?orderId=${i.orderId}'/>">Đã nhận hàng</a></td>
							</c:if>
							<c:if test="${i.prostatus ==0 }">
										<td>Chờ chuẩn bị hàng</td>
							</c:if>
			...
			<div class="card border-secondary mb-5">
				<div class="card-footer border-secondary bg-transparent">
					<div class="card-header bg-secondary text-center border-0">
						<a type="button" class="btn btn-block btn-default font-weight-semi-bold" href="orderhistory">ORDER HISTORY</a>
					</div>
					<hr/>
					<div class="card-header bg-secondary text-center border-0">
